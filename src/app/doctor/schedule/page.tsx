...
interface ScheduleSummary {
    ...
    morning_shifts_count: number;
    evening_shifts_count: number;
}
...
// ─── Shift Config ─────────────────────────────────────────────
function shiftConfig(type: string | null) {
    const t = type?.trim();
    if (t === "صباحية" || t === "morning") return {
...
export default function SchedulePage() {
    const { data: scheduleResponse, isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            ...
            return res.data as WeeklyScheduleResponse;
        } 
    });

    if (isLoading) return <ScheduleSkeleton />;
    if (!scheduleResponse) return <ErrorData refetch={refetch} />

    const { summary, schedule } = scheduleResponse;
    const todayRow = schedule.find((d: ScheduleItem) => d.day === todayEn);
    const todayCfg = todayRow ? shiftConfig(todayRow.shift_type) : null;
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-5 sm:space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500" dir="rtl">
            ...
            {todayRow && todayCfg && (
                <motion.div
                    ...
                    className={cn(
                        ...
                        todayRow.status === "إجازة" ? "border-border/50" : "border-primary/20"
                    )}
                >
                    ...
                    <div className="p-4 sm:p-5 flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="font-black text-sm sm:text-base text-foreground">{dayMapping[todayRow.day] || todayRow.day}</p>
                        </div>
                        {todayRow.status !== "إجازة" && (
            ...
            <div className="grid grid-cols-4 gap-2 sm:gap-3">
                {[
                    { label: "أيام العمل", value: summary.total_working_days, suffix: "يوم", color: "text-primary", bg: "bg-primary/8 border-primary/15" },
                    { label: "إجمالي الساعات", value: Math.abs(summary.total_working_hours), suffix: "ساعة", color: "text-emerald-600", bg: "bg-emerald-500/8 border-emerald-500/15" },
                    { label: "وردية صباحية", value: summary.morning_shifts_count, suffix: "أيام", color: "text-amber-600", bg: "bg-amber-500/8 border-amber-500/15" },
                    { label: "وردية مسائية", value: summary.evening_shifts_count, suffix: "أيام", color: "text-blue-600", bg: "bg-blue-500/8 border-blue-500/15" },
                ].map((stat, i) => (
                    <motion.div
                        key={stat.label}
            ...
            >
                ...
                <div className="grid grid-cols-12 gap-1 sm:gap-2 px-3 sm:px-6 py-2 bg-muted/10">
                    ...
                    <span className="hidden sm:block sm:col-span-3 text-[10px] sm:text-xs font-bold text-muted-foreground uppercase tracking-wider">ساعات العمل</span>
                </div>

                <div className="divide-y divide-border/30">
                    {schedule.map((row: ScheduleItem, i: number) => {
                        const isToday = row.day === todayEn;
                        const cfg = shiftConfig(row.shift_type);
                            ...
                            >
                                ...
                                <div className="col-span-4 sm:col-span-3 flex items-center gap-1 sm:gap-1.5 min-w-0">
                                    <span className={cn(
                                        ...
                                        isToday ? "text-primary" : row.status === "إجازة" ? "text-muted-foreground/60" : "text-foreground"
                                    )}>
                                        {dayMapping[row.day] || row.day}
                                    </span>
                                    {isToday && (
                                ...
                                <div className="col-span-4 sm:col-span-3 flex items-center justify-center gap-0.5 sm:gap-1">
                                    {row.status === "إجازة" ? (
                                        ...
                                        <>
                                            <span className="font-mono font-bold text-[10px] sm:text-sm text-foreground" dir="ltr">{row.start_time}</span>
                                            <span className="text-muted-foreground/40 text-[10px] sm:text-sm rotate-180">→</span>
                                            <span className="font-mono font-bold text-[10px] sm:text-sm text-foreground" dir="ltr">{row.end_time}</span>
                                        </>
                                    )}
                                </div>
                                ...
                                <div className="hidden sm:block sm:col-span-3">
                                    {row.daily_hours !== 0 && (
                                        <span className="text-xs sm:text-sm text-muted-foreground font-medium truncate block">
                                            {Math.abs(row.daily_hours)} ساعات
                                        </span>
                                    )}
                                </div>
                            </motion.div>
