...
export default function InventoryStats({ isAdmin = true }: InventoryStatsProps) {
    // جلب البيانات بناءً على الصلاحية
    ...
    const dynamicData = !isAdmin && statsResponse?.data ? statsResponse.data : null;

    const statsData = [
        {
            label: "إجمالي الأصناف",
            value: dynamicData ? dynamicData.total_items.toLocaleString() : "1,240",
            description: isAdmin ? "+12 صنف جديد هذا الشهر" : "إجمالي الأصناف المتوفرة",
            icon: Package,
            color: "text-blue-500 bg-blue-500/10 border-blue-500/20",
            trend: "up"
        },
        {
            label: "القيمة الإجمالية",
            value: dynamicData ? `${Math.round(dynamicData.total_value).toLocaleString()}$` : "450,000$",
            description: "مخزون الأدوية والمستلزمات",
            icon: DollarSign,
            color: "text-emerald-500 bg-emerald-500/10 border-emerald-500/20",
            trend: "up"
        ...
        {
            label: "منخفض المخزون",
            value: dynamicData ? dynamicData.low_stock_count.toString() : "23",
            description: "تحتاج إعادة تعبئة عاجلة",
            icon: AlertTriangle,
            color: "text-orange-500 bg-orange-500/10 border-orange-500/20",
            trend: "down"
        ...
        {
            label: "نفد المخزون",
            value: dynamicData ? dynamicData.out_of_stock_count.toString() : "5",
            description: "غير متوفرة حالياً",
            icon: PackageX,
            color: "text-red-500 bg-red-500/10 border-red-500/20",
            trend: "down"
