...
export function MedicalPointCard({ point, isAdmin = true }: MedicalPointCardProps) {
    ...
    const detailsUrl = isAdmin
        ? `/admin/medical-points/clinics/${point.id}`
        : `/manager/departments/${point.id}`;
    return (
        <Link
            href={detailsUrl}
            ...
            >
                <div className="relative h-48 overflow-hidden bg-muted flex items-center justify-center">
                    {imageError ? (
                        ...
                        <Image
                            src={ "/images/defaultMediacaPoint.jpg"}
                            alt={point.name}
                            fill
                ...
                <div className="p-5 flex-1 flex flex-col">
                    ...
                    <div className="flex items-center gap-1.5 text-muted-foreground text-sm mb-4">
                        ...
                        <span className="truncate">{point.location || "قسم طبي"}</span>
                    </div>
                    ...
                    <div className="mt-auto grid grid-cols-3 gap-2 py-4 border-t border-border/50 text-center">
                        ...
                        <div className="border-r border-border/50">
                            ...
                            <p className="text-xs font-bold">
                                {isAdmin ? (point.departments_count || 0) : (point.total_visits || 0)}
                            </p>
                        </div>
                    </div>
                </div>
