...
export default function AddMedicalPointModal({
    isOpen, 
    onClose, 
    title, 
    isAdmin = true, 
    initialData = null 
}: AddMedicalPointModalProps) {
    ...
    const { data: managers = [] } = useQuery({
        queryKey: ["region-managers"],
        queryFn: async () => {
            const res = await api.get("/region-managers");
    ...
    const displayTitle = title || (isEdit ? `تعديل ${nameForm}` : `إضافة ${nameForm}`);

    // Add Mutation
    const addMutation = useMutation({
        mutationFn: async (data: any) => {
            const res = await api.post("/medical-points", data);
            return res.data;
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["medical-points"] });
            setShowSuccess(true);
            toast.success("تمت العملية بنجاح", {
                description: "تمت إضافة النقطة الطبية بنجاح.",
                className: "font-sans",
            });
            setTimeout(() => {
                handleClose();
        ...
        onError: (error: any) => {
            toast.error("فشلت العملية", {
                description: error?.response?.data?.message || "حدث خطأ أثناء إضافة النقطة الطبية.",
            });
        }
    });
    ...
    useEffect(() => {
        if (isOpen && initialData) {
            setFormData({
                ...
                regions_id: "", // Needs proper mapping if editing
                managers_id: "",
    ...
    function handleValidateModal() {
        ...
        if (!formData.regions_id) {
            newErrors.regions_id = "الرقم التعريفي للمنطقة مطلوب";
        }

        if (!formData.managers_id) {
            newErrors.managers_id = "الرقم التعريفي للمدير مطلوب";
        }

        if (Object.keys(newErrors).length > 0) {
            setErrors(newErrors);
            toast.error("بيانات غير مكتملة", {
                description: "يرجى تعبئة كافة الحقول الإجبارية.",
                className: "font-sans",
            });
            return;
        }
        ...
        addMutation.mutate({
            name: formData.name,
            regions_id: parseInt(formData.regions_id),
            managers_id: parseInt(formData.managers_id),
                            ...
                            {!showSuccess ? (
                                ...
                                >
                                    ...
                                    <div className="p-6 border-b border-border/50 bg-linear-to-br from-primary/5 to-transparent relative">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <h2 className="text-xl font-black text-foreground tracking-tight">{displayTitle}</h2>
                                                <p className="text-xs text-muted-foreground font-bold opacity-70">
                                                    {isEdit ? "قم بتحديث البيانات المطلوبة" : "أدخل البيانات بعناية لإنشاء السجل"}
                                                </p>
                                            </div>
                                        </div>
                                        <button
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                            ...
                                            <div className="space-y-2">
                                                <label className="text-xs font-black text-muted-foreground flex items-center gap-2">
                                                    ...
                                                    <span>اختر المدير</span>
                                                </label>
                                                <select
                                                    value={formData.managers_id}
                                                    onChange={(e) => {
                                                        setFormData({ ...formData, managers_id: e.target.value });
                                                        if (errors.managers_id) setErrors({ ...errors, managers_id: "" });
                                                    }}
                                                    className={cn(
                                                        "w-full h-12 rounded-2xl bg-muted/20 border border-border/30 focus:bg-background transition-all font-bold px-4 outline-hidden appearance-none cursor-pointer",
                                                        errors.managers_id && "border-destructive/50 ring-destructive/20 focus:border-destructive"
                                                    )}
                                                >
                                                    <option value="" disabled>اختر المدير المسئول...</option>
                                                    {managers.map((m: any) => (
                                            ...
                                            <div className="space-y-2">
                                                <label className="text-xs font-black text-muted-foreground flex items-center gap-2">
                                                    ...
                                                    <span>معرف المنطقة (ID)</span>
                                                </label>
                                                <Input
                                                    type="number"
                                                    value={formData.regions_id}
                                                    onChange={(e) => {
                                                        setFormData({ ...formData, regions_id: e.target.value });
                                                        if (errors.regions_id) setErrors({ ...errors, regions_id: "" });
                                                    }}
                                                    placeholder="مثال: 1"
                                                    className={cn(
                                                        "h-12 rounded-2xl bg-muted/20 border-border/30 focus:bg-background transition-all font-bold",
                                                        errors.regions_id && "border-destructive/50 ring-destructive/20 focus:border-destructive"
                                                    )}
                                                />
                                                {errors.regions_id && (
                                                    <p className="text-[10px] font-bold text-destructive flex items-center gap-1">
                                    ...
                                    <div className="p-6 bg-muted/30 border-t border-border/50 flex items-center justify-end gap-3 mt-auto">
                                        ...
                                        <button
                                            ...
                                            type="button"
                                        >
                                            {isEdit ? <CheckCircle2 size={18} /> : <Plus size={18} />}
                                            <span>{addMutation.isPending ? "جاري الإرسال..." : (isEdit ? "حفظ التغييرات" : `إضافة ${nameForm}`)}</span>
                                        </button>
                                    </div>
                                </motion.div>
                            ) : (
                                ...
                                >
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        >
                                            {isEdit 
                                                ? `تم تحديث بيانات ${nameForm} بنجاح.` 
