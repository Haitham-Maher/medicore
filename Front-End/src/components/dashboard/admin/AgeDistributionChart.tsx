...
export default function AgeDistributionChart() {
    ...
    const { data: response, isLoading } = useQuery({
        queryKey: ["chart age"],
        queryFn: async () => {
            const res = await api.get("/dashboard/charts/age-distribution");
    ...
    // لنفترض أن البيانات القادمة من الـ API اسمها response
    ...
    // تحويل الـ Object إلى Array of Objects
    const formattedData = rawData ? Object.keys(rawData).map((day) => ({
        name: dayTranslation[day] || day,                // ترجمة اليوم أو إبقاءه كما هو إذا لم يوجد في الخريطة
        children: rawData[day].child,
        adults: rawData[day].adult,
        elderly: rawData[day].elderly,
    })) : [];
