...
interface InventoryDesktopTableProps {
    ...
    search: string;
    onDelete: (id: number) => void; // تم تعديل النوع إلى number ليتناسب مع الـ API
}
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        <AnimatePresence initial={false}>
                            {data.length > 0 ? (
                                data.map((item) => {
                                    // تم التعديل لاستخدام item.type بدلاً من item.category
                                    const Icon = getCategoryIcon(item.type);

                                    // استنتاج الحالة بناءً على الكمية (لأن API لا يرسل status)
                                        ...
                                        >
                                            ...
                                            <TableCell className="py-3 lg:py-4 pr-4 lg:pr-0">
                                                <div className="flex items-center gap-2 lg:gap-3">
                                                    ...
                                                    <div className="flex flex-col min-w-0">
                                                        ...
                                                        {/* تم استبدال تاريخ الانتهاء الوهمي باسم المخزن الحقيقي */}
                                                        <span className="text-[9px] lg:text-[10px] text-muted-foreground/60 font-bold italic tracking-tighter">مخزن: {item.storage_name}</span>
                                                    </div>
                                                </div>
                                            </TableCell>
                                            ...
                                            <TableCell>
                                                <div className="flex items-center gap-1.5 lg:gap-2">
                                                    ...
                                                    {/* تم إزالة item.unit لأنها غير موجودة بالبيانات، واستبدالها بنص ثابت أو تركها فارغة */}
                                                    <span className="text-[9px] lg:text-[10px] text-muted-foreground font-bold lg:font-black opacity-40">وحدة</span>
                                                </div>
                                            </TableCell>
