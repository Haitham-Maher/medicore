...
interface MedicalPointHeaderProps {
    ...
    image: string;
    manager?: string; // Added to support editing
    isLoading?: boolean;
}

export default function MedicalPointHeader({ name, location, rating, status, image, manager = "د. محمد حسن", isLoading = false }: MedicalPointHeaderProps) {
    if (isLoading) {
        return (
